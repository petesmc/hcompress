...
static int encode(unsigned char *outfile, long *nlen, int a[], int nx, int ny, int scale);
...
static int encode64(unsigned char *outfile, long *nlen, LONGLONG a[], int nx, int ny, int scale);
static void shuffle64(LONGLONG a[], int n, int n2, LONGLONG tmp[]);

static  void writeint(unsigned char *outfile, int a);
static  void writelonglong(unsigned char *outfile, LONGLONG a);
static  int doencode(unsigned char *outfile, int a[], int nx, int ny, unsigned char nbitplanes[3]);
static  int doencode64(unsigned char *outfile, LONGLONG a[], int nx, int ny, unsigned char nbitplanes[3]);
static int  qwrite(char *file, char buffer[], int n);

static int qtree_encode(unsigned char *outfile, int a[], int n, int nqx, int nqy, int nbitplanes);
static int qtree_encode64(unsigned char *outfile, LONGLONG a[], int n, int nqx, int nqy, int nbitplanes);
static void start_outputing_bits(void);
static void done_outputing_bits(unsigned char *outfile);
static void output_nbits(unsigned char *outfile, int bits, int n);
...
static void write_bdirect(unsigned char *outfile, int a[], int n,int nqx, int nqy, unsigned char scratch[], int bit);
static void write_bdirect64(unsigned char *outfile, LONGLONG a[], int n,int nqx, int nqy, unsigned char scratch[], int bit);

/* #define output_nybble(outfile,c)	output_nbits(outfile,c,4) */
static void output_nybble(unsigned char *outfile, int bits);
static void output_nnybble(unsigned char *outfile, int n, unsigned char array[]);
...
static unsigned char code_magic_u[2] = { 0xDD, 0x99 };

/* ######################################################################### */
static int encode(unsigned char *outfile, long *nlength, int a[], int nx, int ny, int scale)
{
...
/* ######################################################################### */
static int encode64(unsigned char *outfile, long *nlength, LONGLONG a[], int nx, int ny, int scale)
{
...
/* ######################################################################### */
static void
writeint(unsigned char *outfile, int a)
{
int i;
unsigned char b[4];
...
/* ######################################################################### */
static void
writelonglong(unsigned char *outfile, LONGLONG a)
{
int i;
unsigned char b[8];
...
/* ######################################################################### */
static int
doencode(unsigned char *outfile, int a[], int nx, int ny, unsigned char nbitplanes[3])
{
/* unsigned char *outfile;						 output data stream
...
/* ######################################################################### */
static int
doencode64(unsigned char *outfile, LONGLONG a[], int nx, int ny, unsigned char nbitplanes[3])
{
/* unsigned char *outfile;						 output data stream
...
static void
output_nbits(unsigned char *outfile, int bits, int n)
{
...
static void
output_nybble(unsigned char *outfile, int bits)
{
	/*
...
static void
done_outputing_bits(unsigned char *outfile)
{
	if(bits_to_go2 < 8) {
...
/* ######################################################################### */
static int
qtree_encode(unsigned char *outfile, int a[], int n, int nqx, int nqy, int nbitplanes)
{
...
unsigned char *scratch, *buffer;
	 ...
	 */
	scratch = (unsigned char *) calloc(2*bmax, sizeof(unsigned char));
	...
	for (bit=nbitplanes-1; bit >= 0; bit--) {
		 ...
		 */
		for (k = 1; k<log2n; k++) {
			qtree_reduce(scratch,ny,nx,ny,scratch); //scratch=[2,0,0,0] ny=1  nx=3
			nx = (nx+1)>>1;
			ny = (ny+1)>>1;
			if (bufcopy(scratch,nx*ny,buffer,&b,bmax)) { 
				//nx=2, ny=1, b=0, bmax=2, scratch=[8,0,0,0], to_go = 3, bitbuffer=1, bitcount=56
				//nx=1, ny=1, b=0, bmax=2, scrathc=[8,0,0,0], to_go=6, bitvuffer=25
				write_bdirect(outfile,a,n,nqx,nqy,scratch,bit);
				goto bitplane_done;
			}
		}
...
/* ######################################################################### */
static int
qtree_encode64(unsigned char *outfile, LONGLONG a[], int n, int nqx, int nqy, int nbitplanes)
{
...
/* ######################################################################### */
static void
write_bdirect(unsigned char *outfile, int a[], int n,int nqx, int nqy, unsigned char scratch[], int bit)
{
...
/* ######################################################################### */
static void
write_bdirect64(unsigned char *outfile, LONGLONG a[], int n,int nqx, int nqy, unsigned char scratch[], int bit)
{
